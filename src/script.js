...
$(() => {
    $.get('links.yaml', function (yamlText) {
        ...
        // Map YAML section types to nav anchor IDs
        const sectionIdMap = { developer: 'tech', filmmaker: 'film' };

        // Create sections based on the new structure
        data.sections.forEach(section => {
            const sectionAttrs = { class: 'section' };
            if (sectionIdMap[section.type]) {
                sectionAttrs.id = sectionIdMap[section.type];
            }
            const $section = $('<div>', sectionAttrs);

            // Section header
            const $header = $('<div>', { class: 'section-header' });
            const $title = $('<h1>');
            if (section.icon) {
                if (section.icon.endsWith('.svg')) {
        ...
        $('a:not(.contact-btn):not(.nav-link)').click(function (e) {
            e.preventDefault();
        ...
        // Add click sound (shared via click-sound.js)
        ...
        function updateActiveNav() {
            ...
            const navOffset = $('#site-nav').outerHeight() + 100;
            ...
            $navLinks.each(function () {
                ...
                const $section = $(href);
                if ($section.length) {
                    if ($section.offset().top - navOffset <= scrollPos) {
                        currentId = href;
                    }
                }
            });
            ...
            if (scrollPos + windowHeight >= docHeight - 50) {
                const lastHref = $navLinks.last().attr('href');
                if (lastHref && lastHref.startsWith('#')) currentId = lastHref;
            }

            if (!currentId && $navLinks.length > 0) {
                // Activate first link by default if at top
                currentId = $navLinks.first().attr('href');
            }
